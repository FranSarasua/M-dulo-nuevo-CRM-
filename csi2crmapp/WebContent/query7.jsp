		...
		<div class="container">
			<div class="row">
				<section>
					...
					<br />
					<p>
						Iniciamos nueva tanda con la <strong> Primera Query </strong>. <br /><strong>
						Listado de Incidencias ocurridas en cada mes.</strong> 
					</p>
					<%
						String query7 = "select count(idIncidencia), monthname(fechaincidencia) from incidencias group by(month(fechaincidencia));";
						beanDB basededatos7 = new beanDB();
						String[][] tablares7 = basededatos7.resConsultaSelectA3(query7);
					%>
					<table class="table">
						<%
							for (int i = 0; i < tablares7.length; i++) {
						%>
						<tr>
							<%
								for (int j = 0; j < tablares7[i].length; j++) {
							%>
							<td><%=tablares7[i][j]%></td>
							<%
								}
