		...
		<div class="container">
			<div class="row">
				<section>
					...
					<br />
					<p>
						Seguimos con la <strong> Quinta Query</strong> de la tanda 1. <br /><strong>Cuantas
						horas de clase da cada profesor a la semana.</strong> 
					</p>
					<%
						String query6 = "Select p.nombre, Count(numHoras) from modulos m join profesores p on (p.idProfesor=m.Profesores_idProfesor) group by(idProfesor);";
						beanDB basededatos6 = new beanDB();
						String[][] tablares6 = basededatos6.resConsultaSelectA3(query6);
					%>
					<table class="table">
						<%
							for (int i = 0; i < tablares6.length; i++) {
						%>
						<tr>
							<%
								for (int j = 0; j < tablares6[i].length; j++) {
							%>
							<td><%=tablares6[i][j]%></td>
							<%
								}
